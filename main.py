...
def print_and_return(list_param):
    print(list_param[0])
    return list_param[1]
...
def first_plus_length(list_param):
    return list_param[0] + len(list_param)
...
def values_greater_than_second(list_param):
    if len(list_param) < 2:
        return False
    new_list = []
    for i in list_param:
...
def length_and_value(size_param1, value_param2):
    new_list = []
    for i in range(size_param1):
        new_list.append(value_param2)
    return new_list
